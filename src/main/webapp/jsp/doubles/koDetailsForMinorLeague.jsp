...
<html>
<head>
	<title>KO Round Details for Doubles</title>
	<meta charset="utf-8">
...
<body>
	<div>
		<div data-role="page" data-content-theme="a" id="KORoundInfo" data-title="KO Round info for Doubles 2017">
			...
			<div data-role="content">
				<c:choose>
					<c:when test="${selectedRound == 'R32'}">
						<span style="color:red;font-weight:bold;">Round 32 game should be played between 06/01-06/03</span>
					</c:when>
					<c:when test="${selectedRound == 'R16'}">
						<span style="color:red;font-weight:bold;">Pre-Quarter finals game should be played between 07/23-07/27</span>
					</c:when>
					<c:when test="${selectedRound == 'R8'}">
						<span style="color:red;font-weight:bold;">Quarter finals to be played between 07/28-08/01</span>
					</c:when>
					<c:when test="${selectedRound == 'R4'}">
						<span style="color:red;font-weight:bold;">Semifinals to be played between 08/02-08/04</span>
					</c:when>
					<c:otherwise>
						<span style="color:red;font-weight:bold;">Pre-Quarter finals game should be played between 07/23-07/27</span>
					</c:otherwise>
				...
				<form action="/koRoundForDoubles" method="POST">
					<input type="hidden" name="koType" id="koType" value="Minor"/>
				...
				<%
					...
					for (int row=0; row <playersList.size()-1; row= row+2){
						...
						int gamesDifference = (player1Set1Score+player1Set2Score+player1Set3Score)-(player2Set1Score+player2Set2Score+player2Set3Score);
			...
			<div data-role="footer">
				<span style="text-align:left;font-style:italic;font-size:10px;">For more details please visit <a href="https://sites.google.com/site/beaverpuropentennis/home/season-2017/doubles-tournament/minor-league">Desktop version</a></span>
			</div>
		</div>
	</div>
</body>
