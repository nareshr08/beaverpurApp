...
<script type="text/javascript">
	$(document).ready(function() {
		...
		$('#team1Info').hide();
		//$('#koRoundDiv').hide();
		$('#koType').change(function(event) {
			//alert('changing KO Type');
			$('#formSubmitDiv').hide();
			$('#setScores').hide();
			$('#team2Info').hide();
			$('#team1Info').hide();
			$("#koRound").val("NA");
			$("#koRound").selectmenu('refresh');
		});
		$('#koRound').change(function(event) {
	        ...
	        var $koType = $("select#koType").val();
		 	$.get('/reportScoreForDoublesKO',{koType:$koType,koRound:$koRound},function(responseJson) {
...
<body>
	<div>
		<div data-role="page" data-content-theme="a" id="reportScoreForDoublesKO" data-title="Report scores for KO Doubles 2015">
			...
			<div data-role="content">
				<form action="/reportScoreForDoublesKO" method="POST" id="reportScoreForm">
					...
					<label for="leagueType" class="select">Select League:</label>
					<select name="koType" id="koType">
						<option value="Major" ${selectedKOType == 'Major' ? 'selected="selected"' : ''}>Major League</option>
						<option value="Minor" ${selectedKOType == 'Minor' ? 'selected="selected"' : ''}>Minor League</option>
					</select>
					<div id="koRoundDiv">
